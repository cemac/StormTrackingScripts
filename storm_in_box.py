  ...
  for m in range(6,10):
   for d1 in range(0,30):
    for day in range(1,25):  
     flelist = glob.glob('/nfs/a277/IMPALA/data/4km/precip_tracking_12km_hourly/'+str(yr)+'/a04203*4km_'+str(yr)+'0'+str(m)+str(dys[d1])+'0030-'+str(yr)+'0'+str(m)+str(dys[d1])+'2330_'+str(day)+'.txt')
     for fle in flelist:
        file =  open(fle,'r')
...
# this means the storm spans farther to the west, and east, north and south
 ...
 np.savetxt('CP4_CC_precip_storms_over_box_area_'+str(size_of_storm)+'_lons_'+str(x1)+'_'+str(x2)+'_lat_'+str(y1)+'_'+str(y2)+'.csv', good_days[:], delimiter = ',')
