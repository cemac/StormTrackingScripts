...
Memebers:
    correls(var1, var2, labelstr):
    correl_panel_plot(panelno, figletter, var1c, var2c, var1f, var2f,
                      xlabel, ylabel='99th percentile \n precipitation rate (mm/hr)',
                      panels=[3, 4]):
    shear_pdfs(panelno, figlet, var1c, var1f, xlabel,
               ylabel='Probaility density', panels=[3, 4])
    createvars(csvname)
    histograms(panelno, figlet, var1c, var1f, var2c, var2f, xlab,
               ylab, panels=[2, 2])
.. CEMAC_stomtracking:
...
def shear_pdfs(panelno, figlet, var1c, var1f, xlabel,
    ...
    correl = stat.ttest_ind(var1c, var1f, equal_var=False)
    Sigval = 1.0 - correl[1]
...
class stormstats(object):
    ...
    Members:
        Standard_precip_correl(df_fc, df_cc, figname)
        Standard_shear_pdf(df_fc, df_cc, figname)
        Standard_Correl1(df_fc, df_cc, figname)
        Standard_Correl2(df_fc, df_cc, figname)
        Standard_histograms(df_fc, df_cc, figname)
        Standard_coldpool(df_fc, df_cc, figname)
        Standard_pdfs(df_fc, df_cc, figname)
    '''
    def __init__(self, fc_csv, cc_csv):
        ...
        '''
        self.df_fc = createvars(fc_csv)
        self.df_cc = createvars(cc_csv)
