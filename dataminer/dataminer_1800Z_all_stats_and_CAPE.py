# right, so we want the difference between c30404 and c30403 for TCW
# we also want the shear values
...
# break the try loop
 ...
 try:
	#guaranteed_failsafe = gent('../csvs/PAPER_FC_C2C4_STORMID_1800Z_storms_99p_rainfall_above_1mm_no_midday_rain.csv', delimiter = ',')
	...
	for rw in range(0,storms_to_keep.shape[0]):
#
#	  if str(int(storms_to_keep[rw,8])) in C4_FC_list:
#
	    OLRmin = 300.0
	    ukeep925 = 0.0
	    ukeep650 = 0.0
	    ...
	    goodcheck = 0
#            flelist = glob.glob('/nfs/a299/IMPALA/data/fc/4km/a30428/a30428_A1hr_mean_*_fc4km_'+str(a[:4])+''+str(b[:2])+''+str(c[:2])+'0030-*2330.nc')
#	    if len(flelist) >= 1:
#		fle_umass = iris.load_cube(flelist[0])
#		goodcheck = goodcheck + 1
	
#            flelist = glob.glob('/nfs/a299/IMPALA/data/fc/4km/a30429/a30429_A1hr_mean_*_fc4km_'+str(a[:4])+''+str(b[:2])+''+str(c[:2])+'0030-*2330.nc')
#	    if len(flelist) >= 1:
#		fle_vmass = iris.load_cube(flelist[0])
#		goodcheck = goodcheck + 1

            flelist = glob.glob('/nfs/a299/IMPALA/data/fc/4km/c03236/c03236_A1hr_inst_*4km_'+str(a[:4])+''+str(b[:2])+''+str(c[:2])+'0100-'+str(a[:4])+''+str(b[:2])+'*0000.nc')
            if len(flelist) > 0:
                                goodcheck = goodcheck + 1
                                fle_T15 = iris.load_cube(str(flelist[0]))

            flelist = glob.glob('/nfs/a299/IMPALA/data/fc/4km/f30201/f30201_A3hr_inst_*_fc4km_'+str(a[:4])+''+str(b[:2])+''+str(c[:2])+'0300-*0000.nc')
	    if len(flelist) >= 1:
		fle_u = iris.load_cube(flelist[0])
		goodcheck = goodcheck + 1
            
	    flelist = glob.glob('/nfs/a299/IMPALA/data/fc/4km/f30202/f30202_A3hr_inst_*_fc4km_'+str(a[:4])+''+str(b[:2])+''+str(c[:2])+'0300-*0000.nc')
	    if len(flelist) >= 1:
		fle_v = iris.load_cube(flelist[0])
		goodcheck = goodcheck + 1
      

            flelist = glob.glob('/nfs/a299/IMPALA/data/fc/4km/f30208/f30208_A3hr_inst_*_fc4km_'+str(a[:4])+''+str(b[:2])+''+str(c[:2])+'0300-*0000.nc')
	    if len(flelist) >= 1:
		fle_omega = iris.load_cube(flelist[0])
		goodcheck = goodcheck + 1


            flelist = glob.glob('/nfs/a299/IMPALA/data/fc/4km/f30204/f30204_A3hr_inst_*_fc4km_'+str(a[:4])+''+str(b[:2])+''+str(c[:2])+'0300*0000.nc')
	    if len(flelist) >= 1:
                fle_T = iris.load_cube(flelist[0])
		goodcheck = goodcheck + 1

            flelist = glob.glob('/nfs/a299/IMPALA/data/fc/4km/a04203/a04203_A1hr_mean_*_fc4km_'+str(a[:4])+''+str(b[:2])+''+str(c[:2])+'0030*2330.nc')
	    if len(flelist) >= 1:
		fle_precip = iris.load_cube(flelist[0])
		goodcheck = goodcheck + 1

            flelist = glob.glob('/nfs/a299/IMPALA/data/fc/4km/c30403/c30403_A1hr_inst_*_fc4km_'+str(a[:4])+''+str(b[:2])+''+str(c[:2])+'0100-*0000.nc')
	    if len(flelist) >= 1:
...
#  Shear vector between a & b = (U_a - Ub)i  + (V_a  - V_b)j   , i.e a vector
...
################################################################################################################################################
 ...
 try:
	...
	for rw in range(0,storms_to_keep.shape[0]):
#
#	  if str(int(storms_to_keep[rw,8])) in C4_CC_list:
#
	    OLRmin = 300.0
	    ukeep925 = 0.0
	    ukeep650 = 0.0
